...
else if ( isset($HTTP_POST_VARS['group_update']) )
{
	...
	if ( isset($HTTP_POST_VARS['group_delete']) )
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			//Find all the user_id whom are not admins and belong to that
			//group and the selected forum
			//Fixed and checks out now
			$sql = 'SELECT u.user_id FROM ' . USER_GROUP_TABLE . ' ug
					...
					WHERE u.user_level <> '. ADMIN . " AND ug.group_id = $group_id
					AND aa.forum_id = {$row['forum_id']}";
			
			if (!($userListResult = $db->sql_query($sql)))
			{
				message_die(GENERAL_ERROR, 'Could select users for private access forums.', '', __LINE__, __FILE__, $sql);
			}

			$usersToDelete = '';
			
			//Here, no subqueries causes more loops
			while ($userRow = $db->sql_fetchrow($userListResult))
			{
				//This query looks to see if the current user might be singly
				//associated with the forum and still have access
				//Fixed and checks out
				$sql = 'SELECT u.user_id FROM ' . USER_GROUP_TABLE . ' ug
						INNER JOIN ' . USERS_TABLE . ' u ON ug.user_id = u.user_id
						INNER JOIN ' . GROUPS_TABLE .' g ON g.group_id = ug.group_id
						INNER JOIN '. AUTH_ACCESS_TABLE." aa ON aa.group_id = ug.group_id
						WHERE u.user_id = {$userRow['user_id']} AND g.group_single_user = 1
						AND (aa.auth_view = 1 OR aa.auth_read = 1)
						AND aa.forum_id = {$row['forum_id']}";
				...
				{
					//If we find a row, this user is free to stay...Yay!
				}
				else
				{
					//This poor user can no longer see the forum, goodbye watch
					$usersToDelete .= ($usersToDelete == '')? $userRow['user_id'] : ", {$userRow['user_id']}";
				}
			}
