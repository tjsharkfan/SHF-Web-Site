...
else if ( $group_id )
{
			...
			else 
			{
				if ( ( ( isset($HTTP_POST_VARS['approve']) || isset($HTTP_POST_VARS['deny']) ) && isset($HTTP_POST_VARS['pending_members']) ) || ( isset($HTTP_POST_VARS['remove']) && isset($HTTP_POST_VARS['members']) ) )
				{
					...
					else if ( isset($HTTP_POST_VARS['deny']) || isset($HTTP_POST_VARS['remove']) )
					{
						...
						/****************************************
						 * Begin Update by Billy Sauls on March 07, 2007 to the Forum Notification
						 * Mod.
						 * 
						 * This fix will remove all users whom have private permissions via group
						 * when the users are removed from the group.
						 * 
						 * Select and remove All watch users, belonging to a group_id on 
						 * forums marked private whom:
						 * 
						 * 1)  Are not admins
						 * 2)  Do not have access via single_user_group
						 * 
						 * Sadly, it is the long way around since MySQL doesn't like subqueries
						 * until 4.1.x and we want to be safe since phpBB supports older versions :(
						 */
						
						//First we get a list of private viewed/read forums for the group
						//We can assume that this page only deals with non-single-user groups.
						$sql = 'SELECT f.forum_id FROM ' . FORUMS_TABLE . '
								f INNER JOIN ' . AUTH_ACCESS_TABLE . " aa 
								ON f.forum_id = aa.forum_id
								WHERE aa.group_id = $group_id AND
								(f.auth_view = 2 OR f.auth_read = 2)
								AND (aa.auth_view = 1 OR aa.auth_read = 1)";
						//print "<br> $sql";
						if (!($result = $db->sql_query($sql)))
						{
							message_die(GENERAL_ERROR, 'Could select private access forums.', '', __LINE__, __FILE__, $sql);
						}
						//Loop through each forum and delete users who were in the group and 
						//watching these private forums
						while ($row = $db->sql_fetchrow($result))
						{
							//print "Working on forum_id {$row['forum_id']} with a base of user_id ($sql_in)"; //Print Comment
								//This query looks to see if the current user might be singly
								//associated with the forum and still have access
								$sql = 'SELECT u.user_id FROM ' . USER_GROUP_TABLE . ' ug
										INNER JOIN ' . USERS_TABLE . ' u ON ug.user_id = u.user_id
										INNER JOIN ' . GROUPS_TABLE .' g ON g.group_id = ug.group_id
										INNER JOIN ' . AUTH_ACCESS_TABLE." aa ON aa.group_id = ug.group_id
										WHERE u.user_id IN ($sql_in) AND g.group_single_user = 1
										AND (aa.auth_view = 1 OR aa.auth_read = 1)
										AND aa.forum_id = {$row['forum_id']}";
								
								if (!($tmpResult = $db->sql_query($sql)))
								{
									message_die(GENERAL_ERROR, 'Could select user info on private access forums.', '', __LINE__, __FILE__, $sql);
								}
								if ($tmpRow = $db->sql_fetchrow($tmpResult))
								{
									//If we found a user in a row, they get to stay
									$usersWhoStay[] = $tmpRow['user_id'];
								}
								else
								{
								}
							//print "<br>Found these users who are entitled to single user group access (ie stays) ($usersWhoStay)";
							//Now we must compare the list for  $sql_in vs the people who stay
							$OriginalList = explode(",", $sql_in);
							$usersToDelete = "";
							foreach($OriginalList as $value)
							{
								//Look for it in the users who stay, and if not found, add it to the delete list
								if (!in_array(trim($value), $usersWhoStay) && intval($value) != 0)
									$usersToDelete .= ($usersToDelete == "") ? $value : ", $value";
							}
							
							//print "Users who should be deleted from the forums watch: ($usersToDelete)";
							if ($usersToDelete != '')
							{
								$sql = 'DELETE FROM ' . FORUMS_WATCH_TABLE . "
									WHERE user_id IN ($usersToDelete) AND forum_id = {$row['forum_id']}";
								
								if (!$db->sql_query($sql))
								{
									message_die(GENERAL_ERROR, 'Could delete old forum watches.', '', __LINE__, __FILE__, $sql);
								}
							}
						}
						/***************************************************
						 * End Mod Update By Billy Sauls on March 07, 2007
						 */				
	...
	$user_id = 0;	// JSL MOD for file attachments
	display_upload_attach_box_limits($user_id, $group_id);  // JSL MOD for file attachments
