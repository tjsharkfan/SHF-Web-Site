 ...
 *
 * $Id: gallery_remote2.php,v 1.2 2004/08/28 13:12:06 martin Exp $
 */
?>
<?php

//---------------------------------------------------------
// MOD make gallery remote use phpBB2 user DB for authentication if available
if (file_exists(dirname(__FILE__) . '/classes/phpbb/remote.php' )) {
	include((dirname(__FILE__) . '/classes/phpbb/remote.php'));
	if(isset($phpbb_remote)) {
		if($phpbb_remote) {
...
function gr_login( &$gallery, &$response, &$uname, &$password ) {
	global $GR_STAT, $GR_VER, $GALLERY_EMBEDDED_INSIDE;
	...
	if ($tmpUser) {
		if($GALLERY_EMBEDDED_INSIDE == 'phpBB2') {
			if($gallery->userDB->phpbb_login($uname, $password)) {

			/* DEBUG code.
				$response->setProperty( 'status_text', "U: $uname , P: $password" );
				return 0;
			*/
				// log user in
		...
		if($tmpUser->isCorrectPassword($password)) {
			// log user in
			$gallery->session->username = $uname;
