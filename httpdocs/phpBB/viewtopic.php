...
for($i = 0; $i < $total_posts; $i++)
{
	...
	$temp_url = append_sid("posting.$phpEx?mode=reply&amp;" . POST_TOPIC_URL . "=$topic_id&p=" . $postrow[$i]['post_id']);
	$temp_img = ( $forum_topic_data['forum_status'] == FORUM_LOCKED || $forum_topic_data['topic_status'] == TOPIC_LOCKED ) ? $images['reply_post-locked'] : $images['reply_post'];
	$temp_alt = ( $forum_topic_data['forum_status'] == FORUM_LOCKED || $forum_topic_data['topic_status'] == TOPIC_LOCKED ) ? $lang['Topic_locked'] : $lang['Reply_to_post'];
	$reply_post_img = '<a href="' . $temp_url . '"><img src="' . $temp_img . '" alt="' . $temp_alt . '" title="' . $temp_alt . '" border="0" /></a>';
	$reply_post = '<a href="' . $temp_url . '" title="' . $temp_alt . '">' . $lang['Reply_to_post'] . '</a>';
	...
	$template->assign_block_vars('postrow', array(
		...
		'POST_DATE' => $post_date,
		'POST_SUBJECT' => ( $board_config['post_subject'] != 3 && !empty($post_subject) ) ? $lang['Post_subject'] . ': ' . $post_subject : '',
		'MESSAGE' => $message,
		'SIGNATURE' => $user_sig,
		...
		'QUOTE_IMG' => $quote_img,
		'QUOTE' => $quote,
		'REPLY_POST_IMG' => ( $board_config['post_subject'] == 1 || $board_config['post_subject'] == 2 ) ? $reply_post_img : '',
		'REPLY_POST' => ( $board_config['post_subject'] == 1 || $board_config['post_subject'] == 2 ) ? $reply_post : '',
		'IP_IMG' => $ip_img,
		'IP' => $ip,
