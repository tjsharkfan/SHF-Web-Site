...
switch ( $mode )
{
	...
	case 'vote':
		...
		if ( $HTTP_GET_VARS['p'] )
		{
			$select_sql = ( !$submit ) ? ", p.post_username, pt.post_subject, pt.post_id, u.username" : '';
			$from_sql = ( !$submit ) ? ", " . POSTS_TEXT_TABLE . " pt, " . USERS_TABLE . " u" : '';
			$where_sql = ( !$submit ) ? "AND pt.post_id = p.post_id AND u.user_id = p.poster_id" : '';

			$sql = "SELECT f.*, t.topic_status, t.topic_title" . $select_sql . "
...
if ( $mode == 'newtopic' || ( $mode == 'editpost' && $post_data['first_post'] ) )
{
	...
	$template->assign_block_vars('switch_subject', array());
	...
	if ( $board_config['post_subject'] != 3 ) $template->assign_block_vars('switch_subject', array());
}
