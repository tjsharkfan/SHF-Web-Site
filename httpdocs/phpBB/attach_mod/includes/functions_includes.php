...
function display_upload_attach_box_limits($user_id, $group_id = 0)
{
	...
	if ($userdata['user_level'] != ADMIN && $userdata['user_id'] != $user_id && ($user_id != 0)) 		// JSL MOD, added: && ($user_id != 0) 
	{
		return;
	}

	if (!$user_id && ($user_id != 0))		// JSL MOD added: && ($user_id != 0) 
	{
		return;
	}

	// Return if the user is not within the to be listed Group
	if ($group_id && $user_id != 0) 		// JSL MOD added: && $user_id != 0
	{
	...
	//
   	...
   	{ 
      	...
      	if (!($result = $db->sql_query($sql))) 
      	{ 
        	 message_die(GENERAL_ERROR, 'Couldn\'t get users in this group', '', __LINE__, __FILE__, $sql); 
      	} 
      	while ($row = $db->sql_fetchrow($result)) 
      	{ 
   	...
   	$sql_user_id = ($group_id != -1 && $user_id == 0) ? "IN (" . implode(',', $users_in_group) . ")" : "= " . $user_id . ""; 
   	// END JSL MOD	
	
   	$sql = 'SELECT attach_id 
      	FROM ' . ATTACHMENTS_TABLE . " 
      	WHERE user_id_1 " . $sql_user_id . " 
         	AND privmsgs_id = 0 
      	GROUP BY attach_id"; 
      	// JSL MOD; added: " . $sql_user_id . "
	...
	$template->assign_vars(array(
		...
		'U_UACP' => append_sid($phpbb_root_path . 'uacp.' . $phpEx . '?u=' . $user_id . '&amp;sid=' . $userdata['session_id']),
      	'UPLOADED' => $user_uploaded,  // JSL MOD 
      	'QUOTA' => "Unlimited", // JSL MOD    
//      'QUOTA' => $user_quota, // JSL MOD
