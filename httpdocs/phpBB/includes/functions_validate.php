...
//
function validate_email($email, $second_field = FALSE)
{
	global $db, $lang;
	...
	if ($email != '')
	{
		if (preg_match('/^[a-z0-9&\'\.\-_\+]+@[a-z0-9\-]+\.([a-z0-9\-]+\.)*?[a-z]+$/is', $email))
		{
			...
			// added , user_email_2 after user_email
			// added  OR user_email_2 = '" . str_replace("\'", "''", $email) . "'
			$sql = "SELECT user_email, user_email_2
				FROM " . USERS_TABLE . "
				WHERE user_email = '" . str_replace("\'", "''", $email) . "' OR user_email_2 = '" . str_replace("\'", "''", $email) . "'";
			if (!($result = $db->sql_query($sql)))
			{
			...
			if ($row = $db->sql_fetchrow($result))
			{
				...
				if ($second_field)
				{
					return array('error' => true, 'error_msg' => 'Sorry but that e-mail address is already registered to a user.');		
				}
				else
				{
	...
	if ($second_field)
	{
		return array('error' => true, 'error_msg' => 'The secondary email address you entered is invalid.');
	}
	else
	{
