	...
	$mode == 'register' )
{
	...
	$strip_var_list['email_2'] = 'email_2';
	// END
...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	//
	// ADDED BY JSL - second email field mod
	// added || $email_2 != $userdata['user_email_2']
	if ( $email != $userdata['user_email'] || $email_2 != $userdata['user_email_2'] || $mode == 'register' )
	{
		// ADDED BY JSL - second email field mod
		...
		// ADDED BY JSL - second email field mod
		if (!empty($email_2) && $email_2 != $userdata['user_email_2'])
		{
			$result = validate_email($email_2, TRUE);
			if ( $result['error'] )
			{
				$email_2 = $userdata['user_email_2'];
	...
	if ( !$error )
	{
		...
		if ( $mode == 'editprofile' )
		{
			...
			// ADDED BY JSL - second email field mod
			// added , user_email_2 = '" . str_replace("\'", "''", $email_2) ."' after user_email = '" . str_replace("\'", "''", $email) ."'
			$sql = "UPDATE " . USERS_TABLE . "
				SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) ."', user_email_2 = '" . str_replace("\'", "''", $email_2) ."', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_sig_bbcode_uid = '$signature_bbcode_uid', user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_attachsig = $attachsig, user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_notify_pm = $notifypm, user_popup_pm = $popup_pm, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_active = $user_active, user_actkey = '" . str_replace("\'", "''", $user_actkey) . "'" . $avatar_sql . "
				WHERE user_id = $user_id";
			// END
		...
		else
		{
			...
			// ADDED BY JSL - second email field mod
			// added , email_2 after , email and added , '" . str_replace("\'", "''", $email_2) . "' after , '" . str_replace("\'", "''", $email) . "'
			$sql = "INSERT INTO " . USERS_TABLE . "	(user_id, username, user_regdate, user_password, user_email, user_email_2, user_icq, user_website, user_occ, user_from, user_interests, user_sig, user_sig_bbcode_uid, user_avatar, user_avatar_type, user_viewemail, user_aim, user_yim, user_msnm, user_attachsig, user_allowsmile, user_allowhtml, user_allowbbcode, user_allow_viewonline, user_notify, user_notify_pm, user_popup_pm, user_timezone, user_dateformat, user_lang, user_style, user_level, user_allow_pm, user_active, user_actkey)
				VALUES ($user_id, '" . str_replace("\'", "''", $username) . "', " . time() . ", '" . str_replace("\'", "''", $new_password) . "', '" . str_replace("\'", "''", $email) . "', '" . str_replace("\'", "''", $email_2) . "', '" . str_replace("\'", "''", $icq) . "', '" . str_replace("\'", "''", $website) . "', '" . str_replace("\'", "''", $occupation) . "', '" . str_replace("\'", "''", $location) . "', '" . str_replace("\'", "''", $interests) . "', '" . str_replace("\'", "''", $signature) . "', '$signature_bbcode_uid', $avatar_sql, $viewemail, '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', '" . str_replace("\'", "''", $yim) . "', '" . str_replace("\'", "''", $msn) . "', $attachsig, $allowsmilies, $allowhtml, $allowbbcode, $allowviewonline, $notifyreply, $notifypm, $popup_pm, $user_timezone, '" . str_replace("\'", "''", $user_dateformat) . "', '" . str_replace("\'", "''", $user_lang) . "', $user_style, 0, 1, ";
			// END
...
if( isset($HTTP_POST_VARS['avatargallery']) && !$error )
{
	...
	// ADDED BY JSL - second email field mod
	// added , $email_2 after , email
	display_avatar_gallery($mode, $avatar_category, $user_id, $email, $email_2, $current_email, $coppa, $username, $email, &
		$new_password, &
		$cur_password, $password_confirm, $icq, $aim, $msn, $yim, $website, $location, $occupation, $interests, $signature, $viewemail, $notifypm, $popup_pm, $notifyreply, $attachsig, $allowhtml, $allowbbcode, $allowsmilies, $allowviewonline, $user_style, $user_lang, $user_timezone, $user_dateformat, $userdata['session_id']);
	// END
}
else
{
	...
	$template->assign_vars(array(
		...
		// ADDED BY JSL - second email field mod
		'L_EMAIL_ADDRESS_2' => $lang['Email_address_2'],
		// END
